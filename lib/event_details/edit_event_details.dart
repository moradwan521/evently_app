...
class _EditEventDetailsState extends State<EditEventDetails> {
  ...
  void initState() {
    ...
    // تحديد الـ category الحالي حسب eventCategoryId
    currentIndex = categories.indexWhere((cat) => cat.id == widget.event.eventCategoryId);
    if(currentIndex == -1) currentIndex = 0; // default لو مش لاقي
  }
                      ...
                      InkWell(
                        onTap: () {
                          ...
                          Navigator.pop(
                            context,
                            MaterialPageRoute(
                              builder: (context) => EventDetailsScreen(event: data),
                            ),
                          );
                        },
                        child: Container(
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        EventDataModel updatedData = EventDataModel(
                        ...
                        await FirebaseUtils.updateEvent(updatedData);;

                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => EventDetailsScreen(event: updatedData),
                          ),
                        );
                      }
                    },
